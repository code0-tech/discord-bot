...
const check = async () => {
    ...
    results.forEach(async (doublePackets) => {
        console.log(`[MongoDb Check] found _${doublePackets._id} duplicates, count: ${doublePackets.count}`, '#2');
        ...
        packets.forEach(async (packet) => {
            ...
            console.log(`[MongoDb Check] removed _${packet._id} with Dc id: ${packet.id}`, '#2');
        });
    });
}
